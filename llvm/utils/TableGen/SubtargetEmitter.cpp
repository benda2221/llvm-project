...
void SubtargetEmitter::run(raw_ostream &OS) {
  ...
  // Emit declarations for namespace-specific DFA packetizer methods.
  // These are needed when ProcessorItineraries sets PacketizerNamespace to a
  // non-empty string; the implementation is generated by DFAPacketizerEmitter.
  {
    SmallPtrSet<const Record *, 8> EmittedItinDefs;
    for (const CodeGenProcModel &ProcModel : SchedModels.procModels()) {
      if (!ProcModel.hasItineraries())
        continue;
      if (!EmittedItinDefs.insert(ProcModel.ItinsDef).second)
        continue;
      auto NS = ProcModel.ItinsDef->getValueAsString("PacketizerNamespace");
      if (NS.empty())
        continue;
      OS << "  DFAPacketizer *create" << NS
